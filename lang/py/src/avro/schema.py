...
class BytesDecimalSchema(PrimitiveSchema, LogicalSchema):
  def __init__(self, schema_type, precision, scale, other_props=None):
    if schema_type != 'bytes':
      raise SchemaParseException('Logical type DECIMAL must be backed by fixed or bytes.')

    self._validate({'max_precision': self._max_precision(),
                   'precision': precision,
                   'scale': scale})

    PrimitiveSchema.__init__(self, schema_type, other_props)
    LogicalSchema.__init__(self, 'decimal')
    self.set_prop('precision', precision)
    self.set_prop('scale', scale)

  def _validate(self, validation_props=None):
...
class FixedDecimalSchema(FixedSchema, LogicalSchema):
  def __init__(self, schema_type, precision, scale, size, name, namespace=None, names=None, other_props=None):
    if schema_type != 'fixed':
      raise SchemaParseException('Logical type DECIMAL must be backed by fixed or bytes.')
    if size < 0:
      raise SchemaParseException('Size of a fixed length should be greater than 0')

    self._validate({'max_precision': self._max_precision(size),
                   'precision': precision,
                   'scale': scale,
                   'size': size})
    ...
    if type in PRIMITIVE_TYPES:
      if type == 'bytes':
        if logical_type == 'decimal':
          ...
          return BytesDecimalSchema(type, precision, scale, other_props)
      ...
      if type == 'fixed':
        ...
        if logical_type == 'decimal':
          precision = json_data.get('precision')
          scale = 0 if json_data.get('scale') is None else json_data.get('scale')
          return FixedDecimalSchema(type, precision, scale, size, name, namespace, names, other_props)
